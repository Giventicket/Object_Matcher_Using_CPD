...
# 폴더 경로
folder_path = "order1/one/"
folder_list = ["one","two","three"]

for i in range(1,7) : 
    for j in range(3) :
        folder_path = "order"+str(i)+"/"+str(folder_list[j]+'/')
        # 폴더 내 파일 목록 가져오기
        ...
        out = cv2.VideoWriter("output"+"_order"+str(i)+"_"+str(j)+".mp4", fourcc, 10.0, (width, height))  # 출력 파일명, 코덱, FPS, 크기 설정
